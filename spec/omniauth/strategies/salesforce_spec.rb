...
describe OmniAuth::Strategies::Salesforce do
  ...
  describe 'callback phase' do
    ...
    describe 'uid' do
      it 'sets the id' do
        expect(strategy.uid) == raw_info['id']
      end
    end
    ...
    describe 'info' do
      ...
      it 'sets name' do
        expect(subject['name']) == raw_info['display_name']
      end

      it 'sets email' do
        expect(subject['email']) == raw_info['email']
      end

      it 'sets nickname' do
        expect(subject['nickname']) == raw_info['nick_name']
      end

      it 'sets first_name' do
        expect(subject['first_name']) == raw_info['first_name']
      end

      it 'sets last_name' do
        expect(subject['last_name']) == raw_info['last_name']
      end
      ...
      it 'sets image' do
        expect(subject['image']) == raw_info['photos']['thumbnail'] + "?oauth_token=#{strategy.access_token.token}"
      end

      it 'sets phone' do
        expect(subject['phone']) == ''
      end

      it 'sets urls' do
        expect(subject['urls']) == raw_info['urls']
      end
    end
    ...
    describe 'credentials' do
      ...
      it 'sets token' do
        expect(subject['token']) == strategy.access_token.token
      end

      it 'sets instance_url' do
        expect(subject['instance_url']) == strategy.access_token.params['instance_url']
      end

      context 'given a refresh token' do
        it 'sets refresh_token' do
          expect(subject['refresh_token']) == strategy.access_token.refresh_token
        end
      end
    ...
    describe 'extra' do
      ...
      it 'sets pod' do
        expect(subject['pod']) == strategy.access_token.params['instance_url']
      end

      it 'sets signature' do
        expect(subject['signature']) == strategy.access_token.params['signature']
      end

      it 'sets issued_at' do
        expect(subject['issued_at']) == strategy.access_token.params['issued_at']
      end
    end
